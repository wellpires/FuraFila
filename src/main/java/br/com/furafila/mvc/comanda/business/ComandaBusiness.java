...
public class ComandaBusiness {
	...
	public void gravarComanda(Comanda comanda) throws Exception {
		...
		String strQuery = "INSERT INTO COMANDA " + "(id_comanda," + "codigoPedido," + "id_estabelecimento_FK,"
				+ "id_cliente_FK," + "id_status_FK" + ") VALUES (" + comanda.getIdComanda() + ",'"
				+ comanda.getCodigoPedido() + "'" + "," + comanda.getEstabelecimento().getIdEstabelecimento() + ","
				+ comanda.getCliente().getIdCliente() + "," + comanda.getStatus().getIdStatus() + ")";
	...
	public void alterarStatusComanda(Comanda comanda) throws Exception {
		...
		String strQuery = String.format("UPDATE COMANDA SET id_status_FK = %d  WHERE id_comanda = '%s'",
				comanda.getStatus().getIdStatus(), comanda.getIdComanda());
	...
	public void atualizarDataVenda(Comanda comanda) throws Exception {
		...
		String strQuery = "UPDATE COMANDA SET dataConfirmacao = NOW() WHERE id_comanda = '" + comanda.getIdComanda() + "'";
	...
	public List<List<String>> listarProdutosPorComanda(Comanda comanda) throws Exception {
		...
		String strQuery = "SELECT " + " P.id_produto AS [CODIGO_PRODUTO]," + " P.produto_desc  AS [DESC_PRODUTO],"
				+ " P.valor_unitario AS [PRECO],"
				+ " (SELECT PE.qtd FROM PEDIDOS PE WHERE PE.id_produto_FK = P.id_produto  AND PE.id_comanda_FK = '"
				+ comanda.getIdComanda() + "') AS [QTD]" + " FROM PRODUTO P "
				+ " WHERE P.id_produto IN (SELECT PE.id_produto_FK FROM PEDIDOS PE WHERE PE.id_comanda_FK = '"
	...
	public List<String> buscarEstabelecimentoLocker(Comanda comanda) throws Exception {
		...
		String strQuery = "SELECT (SELECT E.id_estabelecimento FROM ESTABELECIMENTO E WHERE E.id_estabelecimento IN (SELECT C.id_estabelecimento_FK FROM COMANDA C WHERE C.id_comanda = "
				+ comanda.getIdComanda() + ")) AS ESTABELECIMENTO,"
				+ " (SELECT id_locker FROM LOCKER L WHERE L.id_locker IN (SELECT PL.id_pedido_FK FROM PEDIDO_LOCKER PL WHERE PL.id_pedido_FK IN (SELECT P.id_pedido FROM PEDIDOS P WHERE P.id_comanda_FK = "
				+ comanda.getIdComanda() + ")))"
				+ " AS [ID_LOCKER] FROM PEDIDO_LOCKER PL WHERE PL.id_pedido_FK IN (SELECT P.id_pedido FROM PEDIDOS P WHERE P.id_comanda_FK = "
				+ comanda.getIdComanda() + ")";
