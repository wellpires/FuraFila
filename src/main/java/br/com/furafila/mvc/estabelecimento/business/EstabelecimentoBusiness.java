...
public class EstabelecimentoBusiness {
	...
	public int gravar(Estabelecimento estabelecimento) throws Exception {

		String strQuery = "INSERT INTO " + "ESTABELECIMENTO" + "(" + "razao_social, " + "email," + "cnpj, "
				+ "inscricao_estadual, " + "status " + ") VALUES (" + "'" + estabelecimento.getRazaoSocial() + "',"
				+ "'" + estabelecimento.getEmail() + "'," + "'" + estabelecimento.getCnpj() + "'," + "'"
				+ estabelecimento.getInscricaoEstadual() + "'," + "'" + estabelecimento.getStatusSQL() + "')";

		BancoDados.executaComando(strQuery);

		List<String> regitros = BancoDados.retornaRegistro(String.format(
				"select e.id_estabelecimento from estabelecimento e where e.cnpj = '%s'", estabelecimento.getCnpj()));

		return Integer.parseInt(regitros.get(0));
	...
	public List<String> listarInformacoesEstabelecimento(Estabelecimento estabelecimento) throws Exception {
		...
		String strQuery = "SELECT " + "E.id_estabelecimento AS [CODIGO], " + "E.razao_social AS [RAZAO_SOCIAL], "
				+ "E.email AS [EMAIL]," + "E.cnpj AS [CNPJ], " + "E.inscricao_estadual AS [INSCRICAO_ESTADUAL], "
	...
	public List<String> pegarEmail(Estabelecimento estabelecimento) throws Exception {
		...
		String strQuery = "SELECT E.email AS [EMAIL] " + "FROM ESTABELECIMENTO E WHERE E.email LIKE '"
	...
	public List<String> pegarCnpj(Estabelecimento estabelecimento) throws Exception {
		...
		String strQuery = "SELECT E.cnpj AS [CNPJ] " + "FROM ESTABELECIMENTO E WHERE E.cnpj = "
	...
	public List<List<String>> listarEstabelecimentoMaisVendem() throws Exception {
		...
		String strQuery = "SELECT " + "E.razao_social AS [EMPRESA]," + " COUNT (P.qtd) AS [PRODUTOS] "
				+ "FROM PEDIDOS P " + "INNER JOIN COMANDA C ON (P.id_comanda_FK = C.id_comanda) "
